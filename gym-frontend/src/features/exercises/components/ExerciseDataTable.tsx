...
export function ExerciseDataTable({
  ...
  const columns: ColumnDef<Exercise>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="text-base">{row.original.muscleGroup}</span>
      ),
    },
    {
      ...
      cell: ({ row }) => {
        ...
        if (difficulty === "BEGINNER") {
          variant = "default"
          customClass = "bg-green-700 text-green-400"
        ...
        return (
          <Badge
            variant={variant}
            className={customClass ? customClass + " text-sm" : "text-sm"}
          >
          ...
          <div className="mb-3 flex flex-wrap items-center justify-between gap-3 border border-border bg-muted/40 px-4 py-2.5">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <Button
                ...
                size="sm"
                onClick={() =>
                  toast("Exportar iniciado", {
                    description: `Exportando ${selectedCount} ejercicios a CSV.`,
                  })
                }
              >
